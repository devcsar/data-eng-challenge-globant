...
class APIPipelines:
    ...
    async def ingest_hired_employes_csv(
        self, session: Session, file: File
    ) -> Tuple[bool, str]:
        """
        ...
        validation, csv_data = await self.rw_ops.read_stream_chunks(
            file, self.ROWS_LIMIT, self.STREAM_FILE_CHUNKS_SIZE_KB
        )
        if not validation:
            return (
                validation,
                "The selected file exceeds the rows limit. (1000)",
            )
